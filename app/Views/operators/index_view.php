				...
				<div class="flex items-center py-4 px-4 w-full">
					<div class="w-full p-4 bg-white rounded-md shadow-md">
						<div class="flex flex-wrap justify-between items-center">
                            ...
                            <div class="ms-auto">
                                <a href="operators/edit" class="inline-block focus:outline-none bg-brand-500 mt-1 text-white hover:bg-brand-600 hover:text-white  text-md font-medium py-2 px-4 rounded">
                                    Dodaj Operativca
                                </a>
                            </div>
						</div>
                            ...
                            <div class="sm:-mx-6 lg:-mx-8 p-4">
                                <div class="relative overflow-x-auto block w-full sm:px-2 lg:px-4">
                                    <div class="bg-white rounded-md shadow-md">
                                        <div class="py-4">
                                            <table class="w-full" id="operators_table">
...
var tablee = $('#operators_table').DataTable({
    processing: true,
    serverSide: true,
    serverMethod: 'post',
    ajax: {
        url: "<?= base_url() ?>/operators/datatable",
    },
    rowCallback: function( row, data ) {
        $(row).addClass('OperatorsModel_row').attr('data-id', data.id);
    },
    retrieve: true,
    cache: true,
    'columns': [
        ...
        {data: 'full_name', render: function(data,type,row){
            return '<a href="operators/edit/' + row.id + '" class="flex items-center">'+
                        ...
                        '<div class="self-center">'+
                            '<h5 class="text-sm font-semibold text-slate-700 dark:text-gray-400">' + (row.full_name ?? 'operativac bez imena') + '</h5>'+
                        '</div>'+
                    '</a>';
            }
        },
        ...
        {data: null, render: function(data,type,row){
                if(row.id != 0)
                    return '<div class="flex gap-5 justify-center">'+
                                ...
                                '<a href="javascript:;"><i class="icofont-ui-delete text-lg text-red-500 dark:text-red-400 delete_record" data-id="' + row.id + '" data-model="OperatorsModel" data-popup="delete-popup" ></i></a>'+
                            '</div>';
                else
                    return "";
            }
